...
- `View`: отображение данных и генерация пользовательских событий (в этой части не используется в `main.ts`).
- `Presenter`: связывает модели и представления.

В сдаче 8 спринта реализованы и проверяются: модели данных и коммуникационный слой.  
`src/main.ts` содержит только тестирование моделей и `GET` запроса каталога.
